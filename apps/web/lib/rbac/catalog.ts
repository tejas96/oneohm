 ...
 * of these 42 codes so the superadmin role builder has something to list, but
...
/**
 * A union of the 42 literal codes.
 *
 * `as const satisfies` above is load-bearing: `as const` narrows each `code`
 * to its literal so this union is 42 strings, while `satisfies` still checks
