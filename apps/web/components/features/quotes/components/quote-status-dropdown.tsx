...
export const QuoteStatusDropdown = React.memo(({
  ...
  const handleSend = (): void => {
    sendMutation.mutate(quoteId, {
      ...
      onError: (err) => {
        showToast.error(
          (err.response?.data as { message?: string } | undefined)?.message ?? 'Failed to send quote',
        );
      },
    });
  };
  ...
  const handleAccept = (): void => {
    ...
    acceptMutation.mutate(
      { quoteId, customerSignature: customerSignature.trim() },
      {
        ...
        onError: (err) => {
          showToast.error(
            (err.response?.data as { message?: string } | undefined)?.message ?? 'Failed to accept quote',
          );
        },
      },
    );
  ...
  const handleReject = (): void => {
    ...
    rejectMutation.mutate(
      { quoteId, rejectionReason: rejectionReason.trim() },
      {
        ...
        onError: (err) => {
          showToast.error(
            (err.response?.data as { message?: string } | undefined)?.message ?? 'Failed to reject quote',
          );
        },
      },
    );
  ...
  return (
    <>
      ...
      {/* Accept Modal */}
      <Dialog open={acceptModalOpen} onOpenChange={(open: boolean) => { setAcceptModalOpen(open); if (!open) setCustomerSignature(''); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Accept Quote</DialogTitle>
            <DialogDescription>Confirm that the customer has accepted this quote.</DialogDescription>
          </DialogHeader>
      ...
      {/* Reject Modal */}
      <Dialog open={rejectModalOpen} onOpenChange={(open: boolean) => { setRejectModalOpen(open); if (!open) setRejectionReason(''); }}>
        <DialogContent>
          <DialogHeader>
