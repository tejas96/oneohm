...
import { getDefaultCostComponentsRecord } from '../constants';
...
export function EditPricingTierModal({
  ...
  const form = useForm<InstallationPricingFormData>({
    ...
    defaultValues: {
      ...
      gstRate: 18,
      costComponents: getDefaultCostComponentsRecord(),
      effectiveFrom: '',
      effectiveTo: '',
      isActive: true,
    },
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      costComponents: ((): Record<string, number> => {
        const raw = target.costComponents ?? {};
        const normalized = Object.fromEntries(
          Object.entries(raw).map(([k, v]) => [k, Number(v) || 0]),
        ) as Record<string, number>;
        return Object.keys(normalized).length > 0 ? normalized : getDefaultCostComponentsRecord();
      })(),
        ...
        <MUIDialogBody sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          ...
          <Card>
            <CardContent className="space-y-4">
              ...
              <CostComponentsEditor form={form} />
            </CardContent>
          </Card>
