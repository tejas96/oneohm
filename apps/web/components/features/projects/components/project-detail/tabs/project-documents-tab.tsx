...
export const ProjectDocumentsTab = React.memo(
  ({ projectId, isActive }: ProjectDocumentsTabProps): React.JSX.Element => {
    const {
      data: documents,
      isLoading,
      isError,
      error,
      refetch,
    } = useProjectDocuments(projectId, { enabled: isActive });
                  ...
                  {(grouped[category] ?? []).map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <FileText
                          className={`size-5 shrink-0 ${colors?.text ?? 'text-foreground-muted'}`}
                        />
