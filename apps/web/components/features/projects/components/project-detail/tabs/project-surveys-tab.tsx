...
function getDocIconStyle(filename: string): { bg: string; icon: typeof FileText; iconColor: string } {
        ...
        <div className="flex flex-wrap gap-2">
          {documents.map((doc) => {
            ...
            return (
              ...
              >
                <div className={`size-10 rounded-full ${style.bg} flex items-center justify-center mb-2`}>
                  <IconComp className={`size-5 ${style.iconColor}`} />
                </div>
                <p className="text-2xs font-medium text-foreground truncate w-full" title={doc.filename}>
                  {doc.filename}
                </p>
                {doc.fileSize > 0 && (
                  <p className="text-[10px] text-foreground-tertiary">{formatBytes(doc.fileSize)}</p>
                )}
...
export const ProjectSurveysTab = React.memo(({
  survey,
}: ProjectSurveysTabProps): React.JSX.Element => {
  if (!survey) {
    return (
      <EmptyState
