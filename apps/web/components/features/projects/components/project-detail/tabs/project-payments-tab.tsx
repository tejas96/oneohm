        ...
        <table className="w-full">
          <thead>
            <tr className="bg-muted/50">
              ...
              <th className="text-2xs font-medium text-foreground-muted uppercase text-left px-3 py-2">Account</th>
          ...
          <tbody className="divide-y divide-border-light">
            {payments.map((payment) => (
              <tr key={payment.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="text-xs text-foreground-secondary px-3 py-2.5 font-mono">
                  {maskAccountNumber(payment.accountNumber)}
                </td>
                <td className="px-3 py-2.5">
                  <Badge
                    variant={(PAYMENT_STATUS_BADGE_VARIANT[payment.status] ?? 'secondary') as 'success'}
                    size="xs"
                  >
