...
export function AddressAutocomplete({
  ...
  const handleSelect = async (suggestion: PlaceSuggestion) => {
    ...
    if (details) {
      // Use the more specific mainText as the street address
  ...
  const handleItemKeyDown = (e: KeyboardEvent<HTMLButtonElement>, suggestion: PlaceSuggestion) => {
    ...
    <div ref={containerRef} className="relative">
      ...
      {isOpen && suggestions.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-background border border-border rounded-lg shadow-lg overflow-hidden">
          <ul role="listbox" aria-label="Address suggestions" className="py-1 max-h-60 overflow-y-auto">
          ...
          <div className="px-3 py-1.5 border-t border-border bg-muted/30">
            <p className="text-xs text-foreground-secondary">Powered by Google Maps</p>
          </div>
        </div>
      )}
    </div>
