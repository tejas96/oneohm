...
export class UpdateMaterialDispatchStatusDto {
  ...
  @ApiProperty({ example: '2024-01-15', description: 'Dispatch date (for IN_TRANSIT status)', required: false })
  @IsDateString()
  @IsOptional()
  dispatchDate?: string;

  @ApiProperty({ example: '2024-01-20', description: 'Actual delivery date (for DELIVERED status)', required: false })
  @IsDateString()
  @IsOptional()
  actualDeliveryDate?: string;

  @ApiProperty({ example: 'uuid', description: 'Received by user ID (for DELIVERED status)', required: false })
  @IsUUID()
  @IsOptional()
  receivedById?: string;
