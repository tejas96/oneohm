...
const OneOhmUI = (function () {
  ...
  const toast = {
    ...
    show(message, type = 'info', duration = 4000) {
      ...
      toast.className =
        'flex items-center gap-3 px-4 py-3 bg-white rounded-xl shadow-lg border border-gray-200 min-w-[300px] max-w-[400px] transform translate-x-full transition-transform duration-300';
      toast.innerHTML = `
  ...
  const modal = {
    show(options = {}) {
      ...
      const icons = {
        success:
          '<div class="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mb-4"><svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></div>',
        error:
          '<div class="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4"><svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></div>',
        warning:
      ...
      backdrop.className =
        'fixed inset-0 bg-black/50 backdrop-blur-sm z-[9998] flex items-center justify-center p-4 opacity-0 transition-opacity duration-200';
    ...
    init() {
      document.addEventListener('click', (e) => {
        ...
        const panelContainer = document.querySelector(
          `[data-tab-panels="${tabGroup.dataset.tabs}"]`,
        );
        if (panelContainer) {
    ...
    overlay(container, show = true) {
      ...
      if (show) {
        if (!overlay) {
          ...
          overlay.className =
            'loading-overlay absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10';
  ...
  const tooltip = {
    init() {
      document.querySelectorAll('[data-tooltip]').forEach((el) => {
        ...
        tip.className =
          'absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 invisible transition-all duration-200 z-50';
